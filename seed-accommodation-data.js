/**
 * Seed Accommodation Groups Data
 * 
 * This script adds accommodationGroups and accommodationTips to existing universities
 * in Firestore. Safe to run multiple times - updates only specified fields.
 * 
 * Usage: node seed-accommodation-data.js
 */
...
const firebaseConfig = {
  apiKey: "AIzaSyBxjYj2PeE_VR51YfAMoYRhWl-lz0Z0s1Y",
  authDomain: "settleu-a8ced.firebaseapp.com",
  projectId: "settleu-a8ced",
  storageBucket: "settleu-a8ced.firebasestorage.app",
  messagingSenderId: "825903082026",
  appId: "1:825903082026:web:f00f6d0c0fb4a5d6093eac",
  measurementId: "G-70ZFPN9XM1"
};
...
// Accommodation data for universities
...
/**
 * Update a university document with accommodation data
 */
async function updateUniversity(universityId, data) {
  try {
    ...
    if (!docSnap.exists()) {
      console.log(`⏭️  Skipping ${universityId} - document does not exist`);
      return { success: false, reason: 'not_found' };
    }

    // Update with accommodation data
    await updateDoc(docRef, {
      accommodationGroups: data.accommodationGroups,
      accommodationTips: data.accommodationTips
    });

    console.log(`✅ Updated ${universityId} with accommodation data`);
    return { success: true };
  } catch (error) {
...
async function seedAccommodationData() {
  ...
  for (const [universityId, data] of Object.entries(accommodationData)) {
    const result = await updateUniversity(universityId, data);
...
// Run the seeding
seedAccommodationData().catch((error) => {
